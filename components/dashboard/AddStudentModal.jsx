  ...
  return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                <div className="card" style={{ width: 480, background: 'var(--surface)' }}>
                          <div className="card-title">Add student (manual entry)</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                  <div className="field"><label>STP code *</label><input value={form.stp_code} onChange={(e) => set('stp_code', e.target.value)} /></div>
                                  <div className="field"><label>Student name *</label><input value={form.student_name} onChange={(e) => set('student_name', e.target.value)} /></div>
                                  ...
                                  <div className="field"><label>Package</label><input value={form.package} onChange={(e) => set('package', e.target.value)} placeholder="Italy / Germany / MBBS / L1E2E" /></div>
                                  <div className="field"><label>Month *</label><input value={form.month} onChange={(e) => set('month', e.target.value)} /></div>
                  ...
                  {err && <div className="error-text">{err}</div>}
                        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 16 }}>
                                  ...
                                  <button className="btn primary" onClick={submit} disabled={busy}>{busy ? 'Adding...' : 'Add student'}</button>
                        </div>
                </div>
        </div>
      );
